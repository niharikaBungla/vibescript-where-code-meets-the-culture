...
class Parser:
    ...
    def statement(self):
        ...
        elif token.type == TOKEN_TYPES['IDENTIFIER'] and self.lexer.peek() == '=':
    ...
    def comparison(self):
        """
        comparison : term ((LESS_THAN | GREATER_THAN | LESS_EQUALS | GREATER_EQUALS) term)*
        """
        node = self.term()
        ...
        while self.current_token.type in (
            ...
            TOKEN_TYPES['LESS_EQUALS'],
            TOKEN_TYPES['GREATER_EQUALS']
        ):
            operator = self.current_token
            self.eat(self.current_token.type)
            right = self.term()
    ...
    def factor(self):
        ...
        while self.current_token.type in (TOKEN_TYPES['MULTIPLY'], TOKEN_TYPES['DIVIDE']):
            operator = self.current_token
            self.eat(self.current_token.type)
            right = self.unary()
            node = BinaryExpression(node, operator, right)
    ...
    def unary(self):
        """
        unary : (MINUS | PLUS) unary
              | primary
        """
        if self.current_token.type in (TOKEN_TYPES['MINUS'], TOKEN_TYPES['PLUS']):
            operator = self.current_token
            self.eat(self.current_token.type)
            operand = self.unary()
